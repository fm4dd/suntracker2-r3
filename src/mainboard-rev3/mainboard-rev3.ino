...
uint8_t dippos2 = 0;         // dipswitch 2 position (motor on)
...
void loop() {
  ...
  if(dippos2 == LOW) {
    if(m1move == false) {
      if(digitalRead(push1) == LOW) { motor1home(); }
      if(digitalRead(push2) == LOW) { motor1led(); }
    }
  }
  /* ------------------------------------------------- */
  /* Set heading LED                                   */
...
/* ------------------------------------------------- */
/* motor1adjust moves from the current position      */
/* to the target position.                           */
/* ------------------------------------------------- */
void motor1adjust(uint16_t target) {
  uint16_t steps = 0;
  m1move = true;
  digitalWrite(ena1,LOW); /* Enable stepstick1 motor driver */
  if(m1cpos < m1tpos) {
